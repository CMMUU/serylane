...
- **TUN 启动与失败恢复（v0.7.17 源码）**：修复预检已通过却不继续启动的问题；切换失败恢复原模式并明确显示结果。macOS 辅助服务按会话隔离，旧服务需主动更新；Windows 保留管理员要求，Linux 暂不提供 TUN。[平台支持与操作说明](docs/TUN启用与兼容性.md)。实际安装包以正式 Release 为准。

- **跨平台应用快选与绑定（v0.7.16）**：Windows MSIX / Store、macOS `.app`、Linux `.desktop` 按应用身份关联；正常更新后重新定位入口。macOS 使用原生启动，Linux 普通桌面入口按独立参数启动；特殊激活／沙箱入口明确标为待适配。缓存即时显示，后台刷新不打断编辑，保留普通文件选择和原参数。[使用与验证说明](docs/应用绑定与自动跟随更新.md)。
...
- [v0.7.17 发布说明](docs/发布说明-v0.7.17.md)
- [TUN 启用与兼容性](docs/TUN启用与兼容性.md)

- [v0.7.13 发布说明](docs/发布说明-v0.7.13.md)
- [路由诊断与双目标稳定策略](docs/路由诊断与双目标稳定策略.md)
...
- 分层连通性诊断
- System Proxy 本地接管与读回验证、活动物理网卡筛选和失败恢复；外网检查在后台完成
