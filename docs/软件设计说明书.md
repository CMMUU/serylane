...
v0.7.13 的路由故障分类、双目标检查及稳定选点以 [路由诊断与双目标稳定策略](路由诊断与双目标稳定策略.md) 为准；替代稳定管理器仅检查 OpenAI API 401 的旧流程，不修改基础 Fallback 或首次候选筛选。

程序代理统一 Windows MSIX、macOS Bundle 与 Linux Desktop Entry 身份，支持缓存展示和后台刷新；各平台独立发现、解析和启动能力检查，macOS 通过 NSWorkspace 新实例注入代理环境，Linux 按 Exec 独立 argv 启动普通入口。展示缓存与启动前真实身份校验分离；详见 [应用绑定与自动跟随更新](应用绑定与自动跟随更新.md)。
